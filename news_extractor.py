...
def news_extractor():
    ...
    save_to_csv(bengal_url, "extracted_links_bengal.csv", BENGAL_LIMIT)
    print("Extracting links from Hindustan Times Sports section...")
    save_to_csv(sports_url, "extracted_links_sports.csv", SPORTS_LIMIT)
    print("Extracting links from Hindustan Times Tech section...")
    save_to_csv(tech_url, "extracted_links_tech.csv", TECH_LIMIT)
    print("Extracting links from Hindustan Times World section...")
    save_to_csv(world_url, "extracted_links_world.csv", WORLD_LIMIT)
    print("Extracting links from Hindustan Times Entertainment section...")
    save_to_csv(entertainment_url, "extracted_links_entertainment.csv", ENT_LIMIT)
    csv_files = [
        ("extracted_links_bengal.csv", "bengal"),
        ("extracted_links_sports.csv", "sports"),
        ("extracted_links_tech.csv", "technology"),
        ("extracted_links_world.csv", "world"),
        ("extracted_links_entertainment.csv", "entertainment")
    ]
    all_links = []
    ...
    if all_links:
        ...
        combined_df.to_csv("extracted_links.csv", index=False, encoding="utf-8")
